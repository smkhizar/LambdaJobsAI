...
## What it does
Turns a job posting into a **1-page, ATS-optimized tailored resume PDF** plus a mandatory matching
1-page cover letter PDF grounded in the candidate's real history, then records the application to SQLite and a
localhost dashboard.
...
| `cover_letter.txt` → `SYED_ALAM_CoverLetter.pdf` | mandatory 1-page cover letter |
...
## Cover letter rules (v3.1)
- Every application gets `cover_letter.txt` and a 1-page PDF.
- Structure: 250-330 words, 3-4 paragraphs, company-specific hook, evidence with JD keywords, fit line, close with US work authorization.
- Optimize for ATS and AI screeners by placing keywords inside real evidence, not bare lists.
- Never use the em dash character. Avoid AI-sounding cliches: "passionate about", "fast-paced environment", "team player", "delve", "leverage" as a verb, "tapestry", "I believe I would be a great fit", "excited to apply".
- Build with `make_cover_letter.py`, confirm `pdfinfo` says 1 page, then re-read extracted text for banned patterns before shipping.
