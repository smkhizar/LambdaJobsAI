# LambdaJobsAI - Agent Instructions (v3.1)
...
tailored resume PDF** + **mandatory 1-page cover letter PDF** → save to SQLite → browse in the dashboard.
...
- `cover_letter.txt` - **MANDATORY for every application** (v3.1); plain text; blank line = new
  paragraph; `**bold**` supported; always built into a 1-page PDF (see cover letter rules below)
...
## Cover Letter Rules (v3.1 - every application gets one, as PDF)
A cover letter must survive three readers: a literal ATS parser, an AI screener scoring
evidence, and a tired human skimming 12 seconds. Rules:

**Structure (250–330 words, 3–4 paragraphs, exactly 1 page PDF):**
1. **Hook (2–3 sentences):** name the company, mirror the EXACT JD title, and say one specific
   thing about their product/mission that could not be pasted into any other letter. No
   "I am excited to apply for" openers.
2. **Evidence (the core):** 2–3 proof points with numbers, each weaving JD keywords into a
   real accomplishment sentence (5,000+ DAU, multi-platform deployments, store releases,
   solo-shipped products). Keywords inside evidence score with AI screeners; bare keyword
   lists read as stuffing and score against you.
3. **Fit line:** why this role shape specifically (backend focus, 0-to-1, mobile, AI product).
4. **Close:** work authorization (US authorized, no sponsorship), location/timezone if
   relevant, one-sentence call to action. No begging, no "I hope".

**Language rules (AI-tell avoidance - human reviewers and detectors both punish these):**
- NEVER use the em dash character. Use periods, commas, parentheses, or short sentences.
- Banned clichés: "passionate about", "fast-paced environment", "team player", "delve",
  "leverage" (as a verb), "tapestry", "I believe I would be a great fit", "excited to apply".
- Vary sentence length; write like a person explaining work to a colleague.
- Every claim must pass the same interview test as resume bullets (Truthfulness Boundary).
- Unique per job. Reusing more than the closing line across letters is a bug.

**Build + verify:** `make_cover_letter.py` auto-fits to exactly 1 page. Check `pdfinfo` = 1 page
and re-read the text for banned patterns before shipping.
